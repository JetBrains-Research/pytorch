...
def disable_saved_tensors_hooks_for_higher_order(
    ...
    def fn(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        # Checked before this transform pushes its own layer: an enclosing
        # Grad/Jvp layer means we are differentiating something already being
        # differentiated (higher-order), where saved tensor hooks miscompute.
        # vmap layers don't differentiate and are ignored.
